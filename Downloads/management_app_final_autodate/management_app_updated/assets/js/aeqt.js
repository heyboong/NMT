/*
 * Interactive spreadsheet logic for the AE‑QT page.
 *
 * This script mirrors the functionality of the AE page but uses a
 * different column configuration (no VNĐ column).  It renders a
 * fixed number of rows, allows inline editing, calculates row totals
 * (money × chia) and a running total across all rows, and persists
 * entries using localStorage.
 */

(function() {
    const tableBody = document.querySelector('#aeqt-table tbody');
    const totalDisplay = document.getElementById('aeqt-total-sum');
    ...
    // Load existing data or initialise with empty objects
    let data = loadData('AEQT_sheet');
    ...
    function computeChia(row) {
        ...
        if (window.FormulaEngine) {
            return window.FormulaEngine.calculate('aeqt', 'chia', row);
        }
    ...
    function computeTT(row) {
        ...
        // Use FormulaEngine if available
        if (window.FormulaEngine) {
            return window.FormulaEngine.calculate('aeqt', 'tt', row) || 0;
        }
        
        // Fallback: original formula (updated to 0.8)
    ...
    function onCellInput(e) {
        ...
        if (col === 'money' || col === 'name') {
            ...
            if (chiaValue === null || typeof chiaValue === 'undefined') {
                ...
                // Multiple names → calculate division, store as rounded number
